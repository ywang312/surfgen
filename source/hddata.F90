...
!$OMP END PARALLEL DO
 ...
 SUBROUTINE writeHd(filename,flheader,writedef)
     ...
     ios = fsync(fnum(SURFOUT))
     if (ios .ne. 0) print *, "*** Error calling FSYNC! ***"     
#else
     ! flush file and write to disk
     call pxffflush(SURFOUT, ios)
     if (ios .ne. 0) print *, "*** Error calling PXFFFLUSH! ***"
#endif
